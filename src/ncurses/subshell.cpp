...
SubShell::SubShell()
{
	...
	if (gethostname((char*)&cHostName, sizeof(cHostName)) != -1)
		_sPromptDivision = _sPromptDivision + "@" + cHostName + ":";
}
...
void	SubShell::SubShellGetConsoleAttr()
{
...
void	SubShell::InitSubShellChild(const string& sPtyName, const string& sDir)
{
    ...
    if (sidLinM != -1) 
    {
		...
		setenv ("MC_SID", strSid.c_str(), 1 );	// MC 와 충동 방지
    }
...
int	SubShell::FeedSubShell(int nHow, bool bOnError)
{
	...
	for (;;)
    {
	    ...
	    /* Read from stdin, Write to the subshell */
		{
			...
			cPtyBuffer[nBytes] = 0;
		    //LOG_WRITE("FeedSubShell STDIN_FILENO [%s] !!!", cPtyBuffer);
